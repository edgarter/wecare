...
class WPML_PO_Parser {
			...
			foreach ( $start_tokens as $key => $start ) {
				while ( $start_pos = strpos( $current_line, $start . '(', $strpos_offset ) ) { //add the opening bracket to the starter
					...
					if ( $end_pos ) {
						...
						if ( sizeof( $call_content_array ) > 1 ) { //actual datetime call has two elements in the gettext string at least
	...
	private static function main_parser( $po ) {
		...
		foreach ( self::find_date_strings( $po_array ) as $key => $value ) {
			$excluded_lines[ ]                 = $value[ 0 ]; // line of the occurrence
			$potential_need_escape_strings [ ] = trim( preg_replace( '/\'/', '', $value[ 1 ][ 0 ] ) ); //datetime string
		}
