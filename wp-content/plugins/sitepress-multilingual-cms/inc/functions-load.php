...
function wpml_load_post_translation( $admin, $settings, $active_language_codes, $default_lang_code ) {
	global $wpml_slug_filter, $wpml_post_translations;

	if ( $admin === true ) {
		$wpml_post_translations = new WPML_Admin_Post_Actions( $settings );
	} else {
		$wpml_post_translations = new WPML_Frontend_Post_Actions( $settings );
...
function wpml_reload_active_languages_setting( $override = false ) {
	...
	if ( $override || wpml_get_setting_filter( false, 'setup_complete' ) ) {
...
function wpml_get_langs_in_dirs_val( $http_client, $wpml_url_converter, $posted_url = false ) {
	...
	return new WPML_Lang_URL_Validator( $http_client, $wpml_url_converter, $posted_url );
}
...
function wpml_get_root_page_actions_obj() {
	...
	if ( !isset( $wpml_root_page_actions ) ) {
		...
		$wpml_root_page_actions = new WPML_Root_Page_Actions();
	}
...
function wpml_load_settings_helper() {
	...
	if ( ! isset( $wpml_settings_helper ) ) {
		require ICL_PLUGIN_PATH . '/inc/setup/wpml-settings-helper.class.php';
		$wpml_settings_helper = new WPML_Settings_Helper();
	}
...
function wpml_load_term_filters() {
	...
	if ( ! isset( $wpml_term_filters_general ) ) {
		...
		$wpml_term_filters_general = new WPML_Term_Filters();
